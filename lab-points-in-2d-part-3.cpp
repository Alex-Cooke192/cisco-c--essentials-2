...
bool Line2D::contains(Point2D point) {
    // Calculate if the inputted point is on the line
    double x = point.getX();
    double y = point.getY(); 
    y = round(y); 

    double y_predicted = (this->slope*x) + this->y_intercept; 
...
int main() {
    ...
    std::getline(std::cin, coordinate_3); 

    stringstream ss1(coordinate_1); 
    stringstream ss2(coordinate_2); 
    stringstream ss3(coordinate_3); 
    ...
    Point2D third_cordinate(coord_3[0], coord_3[1]); 
    ...
    bool collinear_flag = line.contains(third_cordinate); 
